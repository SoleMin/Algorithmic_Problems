...
int input(void) {
	...
	for(i=0;i<lengtha;i++) a[i]=tempa[lengtha-i-1]-'0';
	for(i=0;i<lengthb;i++) b[i]=tempb[lengthb-i-1]-'0';
	return 1;
}
...
int main() {
	...
	while(input()) {
		length[0] = length[1] = 1;
		fib[0][0] = fib[1][0] = 1;
		for(i=1;compare(i%3,a,lengtha)<0;i++) plus((i+1)%3, i%3, (i-1)%3);
		result=i;
		for(;compare(i%3,b,lengthb)<=0;i++) plus((i+1)%3,i%3,(i-1)%3);
		result=i-result;
		printf("%d\n", result);
	}
	return 0;
