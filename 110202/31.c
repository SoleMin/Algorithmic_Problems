...
int main(){
	...
	int hand[2][5];
	long n[2];
	while(gets(card)&&*card){
		int count=0;
		for(int i=0;i<2;i++){
			for(int j=0;j<5;j++){
				while(card[count]==' ')
					count++;
				hand[i][j]=encode_card(card+count);
				count+=2;
			}
			n[i]=get_hand_value(hand[i]);
		}
		if(n[0]<n[1])
			printf("White wins.\n");
		else if(n[0]>n[1])
...
long get_hand_value(int hand[5]){
	...
	if(value[1]+1==value[0]&&suit[1]==suit[0]
		...
		result=(3<<20)+(value[1]<<16)+(value[2]<<8)+(value[3]<<12);
	else if(value[1]==value[2]&&value[3]==value[4])
		result=(3<<20)+(value[0]<<8)+(value[1]<<16)+(value[3]<<12);	
		...
		result=(2<<20)+(value[3]<<16)+(value[1]<<8)+(value[2]<<4)+(value[0]<<12);	
	else
