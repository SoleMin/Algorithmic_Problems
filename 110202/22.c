...
long get_hand_value(int hand[5]){
	...
	else if(((value[0] == value[1] && value[1] == value[2]) && (value[3] == value[4])) || ((value[0] == value[1]) && (value[2] == value[3] && value[3] == value[4]))){
...
void main(void){
	...
	while(fgets(poker, 1024, stdin) != NULL){
		int card = 0;
		...
		for(int i = 0; i < 2; i ++){
			for(int j = 0; j < 5; j ++){
				while(poker[card] == ' ')
					card++;
				getcard[i][j] = encode_card(poker + card);
				card = card + 2;
			}
		}
		
		if(get_hand_value(getcard[0]) > get_hand_value(getcard[1]))
			printf("Black wins.\n");
		
		else if(get_hand_value(getcard[0]) < get_hand_value(getcard[1]))
			printf("White wins.\n");
