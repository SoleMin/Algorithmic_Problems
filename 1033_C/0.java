...
public class Main
{
    ...
    static public void main(String[] args)throws Exception
    {
    	...
    	while(t-->0)
    	{
    		...
    		for(int x=0;x<n;x++)f(x,dp,ar);
    		char c[]=new char[n];
    		for(int x=0;x<n;x++)c[x]=dp[x]==1?'A':'B';
    		sl(new String(c));
    	}
    	p(sb);
    }
    static int f(int i,int dp[],int ar[])
    {
    	if(dp[i]!=-1)return dp[i];
    	for(int x=i+ar[i];x<ar.length;x+=ar[i])if(ar[x]>ar[i]&&f(x,dp,ar)==0)return dp[i]=1;
    	for(int x=i-ar[i];x>=0;x-=ar[i])if(ar[x]>ar[i]&&f(x,dp,ar)==0)return dp[i]=1;
    	return dp[i]=0;
    }
