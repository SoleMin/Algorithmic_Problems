...
void input(void) {
	...
	for (i = 0; i < e; i++) {
		...
		t1 %= 24; // 24시를 0시로 변경
		if ((t1 >= 6 && t1 < 18) || (t1 < 6 && t2 > 6 - t1) || (t1 >= 18 && t2 > 30 - t1)) { // 탈 수 없는 노선정보
			continue;
		}
		// 정오를 0시로 치환하여 저장한다.
...
int getcity(char *name) // 도시 번호 매기는 함수
{
	int i;
	for (i = 0; i < n; i++) {
...
void bfs(void)
{
	...
	queue[rear++] = start;
	check[start][0] = 0; // 0은 필요한 혈액량(여행 날짜)
	check[start][1] = 0; // 1은 제일 빨리 도착할 수 있는 시간
	while (front < rear) {
		now[0] = queue[front++]; // 현재 도시 번호
		now[1] = check[now[0]][0]; // 현재까지 필요 혈액량(리터)
		now[2] = check[now[0]][1]; // 다음 도시에서의 도착 시각
		if (now[0] == finish){ // 이미 도착도시에 있다면 끝내기
			reachable = 1;
			continue;
		}
		for (i = 0; i < ne; i++) { // 입력받은 같은 행선지의 개수만큼 for문
			...
			if (now[2] <= edges[i][2] && // 출발 시각 확인
				...
				// 빈칸 (하루 후에 출발해야하는 상황이라면?)
			else if (now[2] > edges[i][2]&& // 출발 시각 확인
				(check[edges[i][1]][0] > now[1] || // 누적 날짜 확인
				(check[edges[i][1]][0] == now[1] &&
				check[edges[i][1]][1] > edges[i][2] + edges[i][3]))) {
					queue[rear++] = edges[i][1]; // 더 짧은 경로 설정
