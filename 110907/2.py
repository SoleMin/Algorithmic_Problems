...
front = 0
rear = 0
edges = [[0, 0, 0, 0] for _ in range(1000)]
check = [[0, 0] for _ in range(1000)]
city = [0 for _ in range(100)]
queue = [0 for _ in range(10000)]


# breadth first search를 진행한다.
def bfs():
    global front, rear, reachable
    ...
    queue[rear] = start
    check[start][0] = 0  # 필요한 혈액량
    check[start][1] = 0  # 도착할 수 있는 제일 빠른 시간
    rear += 1
    ...
    while front < rear:
        ...
        for i in range(ne):
            ...
            # 출발 시간 확인 and
            if now[2] <= edges[i][2] and (check[edges[i][1]][0] > now[1] or (
                    check[edges[i][1]][0] == now[1] and check[edges[i][1]][1] > edges[i][2] + edges[i][3])):
                queue[rear] = edges[i][1]
                check[edges[i][1]][0] = now[1]
                check[edges[i][1]][1] = edges[i][2] + edges[i][3]
                rear += 1
            # 지금 위치에서 갈 수 있는 곳을 찾는다.
            elif check[edges[i][1]][0] > now[1] + 1 or (
                    check[edges[i][1]][0] == now[1] + 1 and check[edges[i][1]][1] > edges[i][2] + edges[i][3]):
                queue[rear] = edges[i][1]
