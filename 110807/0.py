...
def generate_candidates(puzzle, moves):
    candidates = []
    # 이전 움직임을 가져온다. 없으면 0 1 2 3 4 
    prev_move = moves[-1] if moves else 0

    # 이전에 돌렸던 방향과 반대방향으로 돌리는 것은 의미가 없다.
    x_move=REVERSE_MOVE_TABLE[prev_move]
    if x_move==0:
      candidates.append(((turn1(puzzle)),(1,)))
      candidates.append(((turn2(puzzle)),(2,)))
      candidates.append(((turn3(puzzle)),(3,)))
      candidates.append(((turn4(puzzle)),(4,)))
    elif x_move==1:
...
def generate_solution_cache(puzzle, depth):
    cache = {}
    stack = deque(generate_candidates(puzzle, ()))

    while stack:
        puzzle, moves = stack.popleft()
...
def reverse_seq(moves):
    return tuple(moves[i] for i in range(len(moves)-1,-1,-1))
# 퍼즐을 푼다.
def solve(cache, puzzle, moves=()):
    # 이미 만들어진것이라면
    if puzzle in cache:
        return reverse_seq(reverse_moves(cache[puzzle]))
    ...
    while fifo:
        ...
        if puzzle in cache:
            return moves+reverse_seq(reverse_moves(cache[puzzle]))
