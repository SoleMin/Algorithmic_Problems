...
static int state[24], point[2], count[2], stack[MAXDEPTH], result[MAXDEPTH], rn, solveable, t1, t2;
...
void back(int a) {
	...
	for (i = 0; i < 12 && issame; i++)
		if (state[left(point[0], i)] != finalstate[i]) // 왼쪽원판 
			issame = 0;
	
	for (i = 0; i < 12 && issame; i++)
		if (state[left(point[1], i)] != finalstate[i + 12]) // 오른쪽원판
			issame = 0;
