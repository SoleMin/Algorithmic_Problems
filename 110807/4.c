...
void back(int a) 
{
	...
	{
		if (state[left(point[0], i)] != finalstate[i])	issame = 0;
	}
	for (i = 0; i < 12 && issame; i++)
	{
		if (state[left(point[1], i)] != finalstate[i + 12])	issame = 0;
	}
	//printf("%d %d\n" , a, issame);
	if (issame) 
	{
	...
	for (i = 0; i < 21; i++)
	{
		...
		if (minmove < remainto[i / 2][temp1])	minmove = remainto[i / 2][temp1];
	}
	if (a == MAXDEPTH || a + minmove > MAXDEPTH || a + minmove >= rn)	return;
	...
	for (i = 1; i <= 4; i++) 
	{
		...
		switch (i) 
		{
			case 1:
				...
				for (j = 1; j <= 3; j++)	state[right(point[1], j)] = state[right(point[0], j)];
			...
			case 2:
				...
				for (j = 1; j <= 3; j++)	state[right(point[0], j)] = state[right(point[1], j)];
