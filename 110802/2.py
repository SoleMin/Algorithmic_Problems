...
def backtrack(current_depth, current_x, current_y):
    ...
    if current_depth+Cost>MAXDEPTH:
      return False
    ...
    for i in range(4):
        ...
        # 나는 헷갈려서 movestack이 -1로 초기화하고 위 오른쪽 아래 왼쪽을 각각 0 1 2 3으로 줄래 
        move_x, move_y=move[i]
        if current_depth!=0:
          if abs(movestack[current_depth-1]-i)==2:
            continue
        # 다음좌표가 array를 벗어난다면 다른 이동을 진행시키기 위해 continue
        if current_x+move_x==4 or current_y+move_y==4 or current_x+move_x==-1 or current_y+move_y==-1:
          continue
        # 퍼즐을 이동시킴
        # print(f'현재상태{current_x},{current_y}')
        temp_x=current_x+move_x
        temp_y=current_y+move_y
        temp=puzzle[temp_x][temp_y]
        puzzle[current_x][current_y]=temp
        puzzle[temp_x][temp_y]=0
        # 어디로 이동했는지 저장해줌
        movestack.append(i)
        # print(temp_x,temp_y)
        # 그 후 backtracking 진행
        solved=backtrack(current_depth+1,temp_x,temp_y)
        if solved:
          return True
        else:
          movestack.pop()
          puzzle[temp_x][temp_y]=temp
