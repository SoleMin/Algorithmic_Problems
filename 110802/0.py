...
def backtrack(current_depth, current_x, current_y):
    ...
    for i in range(4):
        ...
        if movestack and (movestack[-1] + 2) % 4 == i:
            continue
        # 다음좌표가 array를 벗어난다면 다른 이동을 진행시키기 위해 continue
        if current_x + move[i][0] < 0 or current_x + move[i][0] > 3 or current_y + move[i][1] < 0 or current_y + \
                move[i][1] > 3:
            continue
        # 퍼즐을 이동시킴
        ...
        puzzle[current_x][current_y], puzzle[next_x][next_y] = puzzle[next_x][next_y], puzzle[current_x][current_y]
        # 어디로 이동했는지 저장해줌
        # 그 후 backtracking 진행
        movestack.append(i)
