...
def backtrack(current_depth, current_x, current_y):
    ...
    for i in range(4):
        ...
        if (len(movestack) > 0 and (movestack[len(movestack) - 1] + 2) % 4 == i ):
          continue;

        nextx = current_x + move[i][0]	
        nexty = current_y + move[i][1]
        # 다음좌표가 array를 벗어난다면 다른 이동을 진행시키기 위해 continue
        if (nextx < 0 or nexty < 0 or nextx >= 4 or nexty >= 4):
          continue;

        # 퍼즐을 이동시킴
        tmp = puzzle[current_x][current_y]
        puzzle[current_x][current_y] = puzzle[nextx][nexty]
        puzzle[nextx][nexty] = tmp
        # 어디로 이동했는지 저장해줌
        # 그 후 backtracking 진행
        movestack.append(i)
        backtrack(current_depth + 1, nextx, nexty)
