...
def find(start):
    ...
    for _ in range(len(cross) - 2):
        ...
        for j in range(len(cross[node])):
            ...
            if dist[j] > cross[node][j] + dist[node]:
                dist[j] = cross[node][j] + dist[node]
...
for _ in range(T):
    ...
    # 설치된 소방서가 있다면 거리 계산 시작
        ...
        fire_dist = list(np.min(fire_dist, axis=0))
        # 만약에 새로운 곳에 소방서를 놓는다면 새로 지어진 곳과 원래 위치의 거리
        ...
        for di in range(len(dist_array)):
            min_dist.append(list(np.min([dist_array[di], fire_dist], axis=0)))
        # 가장 짧은 거리가 가장 긴 곳을 찾아준다.
        for i in range(len(min_dist)):
            min_dist[i] = max(min_dist[i])
        print(min_dist.index(min(min_dist))+1)
    print()
